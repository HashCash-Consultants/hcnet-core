...
small inputs in binary form. We've modified HcNet-core to have a mode for (b)
...
HcNetMessage into them. This is quite a lot of setup and the fuzzing speed of
...
The simplest way is `make fuzz`; this will do the following:
  ...
  - Run `HcNet-core --genfuzz fuzz-testcases/fuzz$i.xdr` ten times to produce
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Add a mode -- with a giant red flashing TESTING_ONLY light on it -- that
    ...
    as an attacker who figures out how to break `HcNet-core` will be able
