...
const struct option HcNet_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: HcNet-core [OPTIONS]\n"
          ...
          "                           Use current as SEQ to catchup to "
          "'current' history checkpoint\n"
          "      --c                  Send a command to local HcNet-core. "
          "try "
          ...
          "      --forcescp           Next time HcNet-core is run, SCP will "
          "start "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("HcNet-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", HcNet_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
