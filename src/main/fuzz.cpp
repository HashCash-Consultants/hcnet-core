 ...
 *     random HcNetMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     HcNet-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(HcNetMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, HcNetMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing HcNet-core " << STELLAR_CORE_VERSION;
