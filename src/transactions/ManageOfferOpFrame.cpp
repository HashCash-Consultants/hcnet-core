...
ManageOfferOpFrame::checkOfferValid(AbstractLedgerTxn& ltxOuter)
{
    ...
    if (sheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = HcNet::loadAccount(ltx, getIssuer(sheep));
        if (!issuer)
        {
    ...
    if (wheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = HcNet::loadAccount(ltx, getIssuer(wheat));
        if (!issuer)
        {
...
ManageOfferOpFrame::doApply(Application& app, AbstractLedgerTxn& ltxOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = HcNet::loadOffer(ltx, getSourceID(), offerID);
        if (!sellSheepOffer)
        {
