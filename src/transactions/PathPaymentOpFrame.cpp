...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerTxn& ltx)
{
    ...
    {
        if (!HcNet::loadAccountWithoutRecord(ltx, mPathPayment.destination))
        {
    ...
    {
        auto destination = HcNet::loadAccount(ltx, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                HcNet::loadAccountWithoutRecord(ltx, getIssuer(curB));
            if (!issuer)
            {
        ...
        auto destLine =
            HcNet::loadTrustLine(ltx, mPathPayment.destination, curB);
        if (!destLine)
        {
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!HcNet::loadAccountWithoutRecord(ltx, getIssuer(curA)))
            {
                innerResult().code(PATH_PAYMENT_NO_ISSUER);
