...
HcNet.org also uses these same github issues to keep track of what we are working on. If you see any issues that are assigned to a particular person that means someone is currently working on that issue. 

Of course feel free to make your own issues if you think something needs to added or fixed.

# Basic quality checks

Please ensure that all tests pass before submitting changes. The local testsuite can be run as `make check` or `src/HcNet-core --test`,
...
All content, comments, and pull requests must follow the [HcNet Community Guidelines](https://www.HcNet.org/community-guidelines/). 
